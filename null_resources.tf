...
resource "null_resource" "Webserver_oci_iscsi_attach" {
 ...
 provisioner "remote-exec" {
    connection {
                type     = "ssh"
                user     = "opc"
                host     = data.oci_core_vnic.Webserver2_VNIC1.public_ip_address
                private_key = file(var.private_key_oci)
                script_path = "/home/opc/myssh.sh"
                agent = false
                timeout = "10m"
...
resource "null_resource" "Webserver1_config_OCFS" {
 depends_on = [oci_core_instance.Webserver1,null_resource.Webserver_oci_iscsi_attach]
 provisioner "remote-exec" {
        connection {
                type     = "ssh"
                user     = "opc"
  ...
  inline = ["echo '== 1. Installing OCFS2'",
            ...
            "sudo o2cb add-node ociocfs2 webserver1 --ip ${data.oci_core_vnic.Webserver1_VNIC1.private_ip_address}",
            "sudo o2cb add-node ociocfs2 webserver2 --ip ${data.oci_core_vnic.Webserver2_VNIC1.private_ip_address}",
...
resource "null_resource" "Webserver2_config_OCFS" {
 depends_on = [oci_core_instance.Webserver2, null_resource.Webserver1_config_OCFS, null_resource.Webserver_oci_iscsi_attach]
 provisioner "remote-exec" {
        connection {
                type     = "ssh"
                user     = "opc"
